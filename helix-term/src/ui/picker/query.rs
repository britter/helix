...
pub(super) fn parse(
    ...
    for ch in input.chars() {
        match ch {
            ...
            '\\' => escaped = !escaped,
            _ if escaped => {
                // Keep backslash escapes for special nucleo syntax.
                if matches!(ch, '^' | '$' | '!' | '\'') {
                    text.push('\\');
                }
                text.push(ch);
...
mod test {
    ...
    fn parse_query_test() {
        ...
        // Quoting
        assert_eq!(
            parse(columns, primary_column, "hello %field1:\"a b c\""),
            hashmap!(
                "primary" => "hello".into(),
                "field1" => "a\\ b\\ c".into(),
            )
        ...
        // Escaping
        assert_eq!(
            parse(columns, primary_column, "hello \\%field1:world"),
            hashmap!(
        ...
        assert_eq!(
            // hello %field1:"a\"b"
            parse(columns, primary_column, "hello %field1:\"a\\\"b\""),
            hashmap!(
                "primary" => "hello".into(),
                "field1" => "a\"b".into(),
            )
